        ...
        {images.map((image, index) => (
          <CarouselItem key={index} className='rounded-lg overflow-hidden'>
            <div className='rounded-lg overflow-hidden'>
                <img src={image.src} alt={image.alt} loading="lazy" className="transform rounded-lg object-fill object-center transition-all duration-500 ease-out group-hover:scale-105" />
            </div>
          </CarouselItem>
        ))}
      </CarouselContent>
