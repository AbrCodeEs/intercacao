...
const HorizontalScrollSection = ({ trans }: Translations) => {
  ...
  return (
    <div className="container mx-auto my-10 flex w-full flex-col px-10">
      ...
      <p className="pb-5 text-center">{trans.description}</p>
      ...
      >
        <div className="flex gap-2 px-4" style={{ minWidth: `${trans.timeline_items.length * 200}px` }}>
          ...
          >
            <a
              ...
              className="flex h-full w-full flex-col items-center justify-center gap-4 rounded-lg border border-gray-300 p-20 text-center"
            >
              <img src={fqp.src} alt="FQP" className="w-20 object-cover" />
              <p className="text-lg font-bold">{trans.explore_section.title}</p>
              <p className="text-lg font-bold">{trans.explore_section.cta}</p>
            </a>
          </motion.div>
        </div>
      </div>
